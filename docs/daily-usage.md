...
- **登录**：平台账号页点击 **扫码登录** → 弹出二维码 → 用 **哔哩哔哩 App** 扫码并在手机确认 → 页面自动提示「登录成功」（无需手动导入 Cookie）
- 首次登录可能稍慢（服务器需准备登录组件），请耐心等待二维码出现
- 若长时间无二维码或登录失败，可点 **检测 Cookie** 查看状态，或联系管理员检查服务器网络
- 发布通过 **biliup** 上传，不支持图文笔记
