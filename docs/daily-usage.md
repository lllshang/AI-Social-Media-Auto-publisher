...
- **开启方式**：在 `ai-publish/.env`（或 Docker 环境变量）设置 `BILIBILI_ENABLED=true`，重启 API 后前台出现 **B站（实验）** 入口；默认 `false` 不影响小红书/抖音/快手
- 发布向导选择 B 站，内容类型固定为**视频**，需选择**投稿分区 tid**
- **登录**：平台账号 → **扫码登录** → 哔哩哔哩 App 扫码（走 biliup，不弹 Chrome）；首次可能需下载 biliup，请稍候
- **发布**：推荐绑定本机 Worker 执行（与小红书混合部署一致）；仅支持视频，不支持图文
- **隔离**：B 站代码与三平台 Adapter 分离；开启 B 站后仍建议各平台各测一次扫码/发布确认无回归
