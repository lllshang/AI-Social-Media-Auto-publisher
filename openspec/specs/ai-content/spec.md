...
Provide pluggable AI text and image generation for social content publishing, with provider auto-detection and runtime switching.
...
- **WHEN** 用户调用 `POST /api/ai/image/generate`，提交 `topic`、`ratio=3:4`、`count=1`
...
- **WHEN** 开发者新增 OpenAI 兼容 Adapter 并注册 provider `openai`
- **THEN** 设置运行时 provider 即可切换

### Requirement: AI 模型自动检测与切换

系统 SHALL 检测本地 Ollama 与远程 API Key 可用性，并支持自动匹配或手动切换文案/文生图模型。

#### Scenario: 启动时自动检测

- **WHEN** API 服务启动
- **THEN** 系统检测可用模型并写入运行时配置，日志输出当前文案与文生图 provider

#### Scenario: 手动切换模型

- **WHEN** 用户调用 `POST /api/ai/models/select`
- **THEN** 系统更新运行时配置并在后续 AI 调用中生效
